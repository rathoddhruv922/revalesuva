...
class DailyReportsViewModel extends GetxController {
  ...
  var listDailyQuestion = <question_model.Datum>[].obs;
  ...
  void loadMore() {
    if (listReport.length < total.value) {
      currentPage.value++;
      fetchDailyReportList(isLoadMore: true);
    }
  }

  fetchDailyReportList({bool isLoadMore = false}) async {
    try {
      if (isLoadMore) {
      ...
      var response = await Repository.instance.getDailyReportListApi(
        perPage: perPage,
  ...
  fetchDailyReportQuestions() async {
    var response = await Repository.instance.getDailyReportQuestionApi(
    ...
    if (response is Success) {
      ...
      listDailyQuestion.assignAll(result.data ?? []);
    } else if (response is Failure) {
      showToast(msg: "${response.errorResponse ?? ""}");
    }
  }
  ...
  addUserAns({
    ...
    final currentQuestion = listDailyQuestion[currentQuestionIndex];
  ...
  Future<bool> submitUserDailyReportAns() async {
    if (listDailyQuestion.length == listCreateUserAns.length) {
      ...
      var response = await Repository.instance.createDailyReportUserAnswerApi(
  ...
  fetchUserDailyReportAns({required String date}) async {
    var response = await Repository.instance.getUserDailyReportListApi(date: date);
    listUserAns.clear();
